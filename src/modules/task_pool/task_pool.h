...
namespace ov
{
	...
	//
	// A task must never wait for another task of this pool to finish: every worker could end
	// up waiting and none would be left to run what they wait for.
	...
	public:
		...
		struct Config
		{
			// Workers to start when a task arrives and no worker is running. Sized for tasks
			// that wait rather than compute, so it does not follow the core count.
			...
			// A worker with no task for this long stops itself
		...
		// the pool is stopped, or when the queue is full and no worker can be added.
		bool Post(Task task);
	...
	protected:
		...
		// Adds `count` workers. The caller holds _mutex.
		...
		// over to be joined, which it cannot do on its own
