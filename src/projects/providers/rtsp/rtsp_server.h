...
private:
    std::vector<std::unique_ptr<RtpTrack>> *FindSessionTracks(const ov::LockGuard<ov::Mutex> &lock, uint32_t stream_id);
    StreamTrackInfo *FindTrackStream(const ov::LockGuard<ov::Mutex> &lock, const std::string &track_path);

    void DeleteStream(const ov::LockGuard<ov::Mutex> &lock, std::unordered_map<std::string, uint32_t>::iterator &stream_id_iterator);
