	...
	while (true)
	{
		...
		if (media_packet->GetFlag() == MediaPacketFlag::Key)
		{
			if (base_pts < media_packet->GetPts())
			{
				...
				logtw("Discovered base PTS value track_id:%d, flags:%d, size:%zu,  pts:%" PRId64 "", (int32_t)media_packet->GetTrackId(), static_cast<int>(media_packet->GetFlag()), media_packet->GetDataLength(), base_pts);
			}
		}
