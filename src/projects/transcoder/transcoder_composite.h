...
private:
	...
	std::shared_ptr<MediaTrack> GetInputTrackByOutputTrackIdLocked(MediaTrackId track_id) const OV_REQUIRES_SHARED(_mutex);
	std::vector<MediaTrackId>   GetFilterIdsByDecoderIdLocked(MediaTrackId decoder_id) const OV_REQUIRES_SHARED(_mutex);
