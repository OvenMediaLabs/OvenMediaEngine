...
TEST(RtmpMediaFrame, AnAvcEndOfSequenceIsNotAFrame)
{
	...
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(Tag({0x17, 0x02, 0x00, 0x00, 0x00})));
}

TEST(RtmpMediaFrame, AVideoInfoCommandFrameIsNotAFrame)
{
	// A command sits where the coded frame would be.
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(Tag({0x57, 0x01, 0x00, 0x00, 0x00})));
}

TEST(RtmpMediaFrame, VideoThisPathCannotServiceIsNotAFrame)
{
	// codec id 2 (H.263)
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(Tag({0x12, 0x01, 0x00, 0x00, 0x00})));
	// codec id 4 (VP6)
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(Tag({0x14, 0x01, 0x00, 0x00, 0x00})));
}
...
TEST(RtmpMediaFrame, AnEnhancedVideoTagHeaderIsNotAFrameOnTheLegacyPath)
{
	...
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(Tag({0x90, 0x61, 0x76, 0x30, 0x31})));
	// `CodedFrames` of `av01`
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(Tag({0x91, 0x61, 0x76, 0x30, 0x31})));
}
...
TEST(RtmpMediaFrame, AudioThisPathCannotServiceIsNotAFrame)
{
	...
	EXPECT_FALSE(pvd::rtmp::CarriesAudioFrame(Tag({0x2F, 0xFF, 0xFB, 0x90})));
	// sound format 7 (G.711 A-law)
	EXPECT_FALSE(pvd::rtmp::CarriesAudioFrame(Tag({0x7F, 0x01, 0x00, 0x00})));
	// sound format 0 (linear PCM)
	EXPECT_FALSE(pvd::rtmp::CarriesAudioFrame(Tag({0x0F, 0x01, 0x00, 0x00})));
}
...
TEST(RtmpMediaFrame, AnEnhancedAudioTagHeaderIsNotAFrameOnTheLegacyPath)
{
	...
	EXPECT_FALSE(pvd::rtmp::CarriesAudioFrame(Tag({0x90, 0x4F, 0x70, 0x75, 0x73})));
	// `CodedFrames` of `Opus`
	EXPECT_FALSE(pvd::rtmp::CarriesAudioFrame(Tag({0x91, 0x4F, 0x70, 0x75, 0x73})));
}

TEST(RtmpMediaFrame, AMessageTooShortToCarryATagHeaderIsNotAFrame)
{
	// Pins the bounds guard; the callers reject a message this short before asking.
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(Tag({})));
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(Tag({0x17})));
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(nullptr));
	EXPECT_FALSE(pvd::rtmp::CarriesAudioFrame(Tag({})));
	EXPECT_FALSE(pvd::rtmp::CarriesAudioFrame(Tag({0xAF})));
	EXPECT_FALSE(pvd::rtmp::CarriesAudioFrame(nullptr));
}
...
{
	EXPECT_TRUE(pvd::rtmp::CarriesAudioFrame(*ExAudio(modules::flv::AudioPacketType::CodedFrames)));
}
...
TEST(RtmpMediaFrame, EveryOtherExAudioPacketTypeIsNotAFrame)
{
	...
	EXPECT_FALSE(pvd::rtmp::CarriesAudioFrame(*ExAudio(modules::flv::AudioPacketType::SequenceStart)));
	EXPECT_FALSE(pvd::rtmp::CarriesAudioFrame(*ExAudio(modules::flv::AudioPacketType::SequenceEnd)));
...
{
	EXPECT_TRUE(pvd::rtmp::CarriesAudioFrame(*LegacyAacAudio(modules::flv::AACPacketType::Raw)));
	EXPECT_FALSE(pvd::rtmp::CarriesAudioFrame(*LegacyAacAudio(modules::flv::AACPacketType::SequenceHeader)));
}

TEST(RtmpMediaFrame, ExVideoCodedFramesIsAFrame)
{
	EXPECT_TRUE(pvd::rtmp::CarriesVideoFrame(*ExVideo(modules::flv::VideoPacketType::CodedFrames)));
	// `CodedFramesX` leaves the composition time offset off the wire.
	EXPECT_TRUE(pvd::rtmp::CarriesVideoFrame(*ExVideo(modules::flv::VideoPacketType::CodedFramesX)));
}

TEST(RtmpMediaFrame, EveryOtherExVideoPacketTypeIsNotAFrame)
{
	// `MPEG2TSSequenceStart` describes an MPEG-2 TS bitstream.
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(*ExVideo(modules::flv::VideoPacketType::SequenceStart)));
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(*ExVideo(modules::flv::VideoPacketType::SequenceEnd)));
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(*ExVideo(modules::flv::VideoPacketType::Metadata)));
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(*ExVideo(modules::flv::VideoPacketType::MPEG2TSSequenceStart)));
	// reserved
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(*ExVideo(static_cast<modules::flv::VideoPacketType>(9))));
}
...
TEST(RtmpMediaFrame, AParsedVideoCommandFrameIsNotAFrame)
{
	...
	EXPECT_FALSE(pvd::rtmp::CarriesVideoFrame(*data));
}
...
TEST(RtmpMediaFrame, BothPathsAgreeOnLegacyAvc)
{
	...
	{
		EXPECT_EQ(pvd::rtmp::CarriesVideoFrame(Tag({0x17, c.packet_type, 0x00, 0x00, 0x00})), c.is_frame)
...
{
	EXPECT_EQ(pvd::rtmp::CarriesAudioFrame(Tag({0xAF, 0x00, 0x12, 0x10})),
			  pvd::rtmp::CarriesAudioFrame(*LegacyAacAudio(modules::flv::AACPacketType::SequenceHeader)));
	EXPECT_EQ(pvd::rtmp::CarriesAudioFrame(Tag({0xAF, 0x01, 0x21, 0x10})),
			  pvd::rtmp::CarriesAudioFrame(*LegacyAacAudio(modules::flv::AACPacketType::Raw)));
}
